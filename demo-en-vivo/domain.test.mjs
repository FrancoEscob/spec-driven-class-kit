...
import {createHabit,markHabit,streak,previousDay,validDate,saveHabits,readHabits} from './checkpoints/shared/domain.mjs';
...
test('A1: crear, marcar y recargar conserva nombre y fecha',()=>{
  ...
  assert.equal(loaded[0].name,'Leer');assert.deepEqual(loaded[0].dates,[today]);
});
test('A2: los espacios externos y las mayúsculas no crean otro hábito',()=>{
  const habits=createHabit([],'Leer','a');
  for(const name of [' leer ','LEER','Leer'])assert.throws(()=>createHabit(habits,name,'b'),/Ya existe/);
  assert.equal(habits.length,1);assert.throws(()=>createHabit(habits,'   ','c'),/nombre/);
...
test('A3: una segunda marca se rechaza sin duplicar ni mutar el estado anterior',()=>{
  const before=createHabit([],'Leer','a'),after=markHabit(before,'a',today,today);
  assert.throws(()=>markHabit(after,'a',today,today),/ya está marcado/);
  assert.deepEqual(before[0].dates,[]);assert.deepEqual(after[0].dates,[today]);
});
test('A4: racha tolerante a que todavía no se haya completado hoy',()=>{
  for(const [dates,expected] of [[[],0],[['2026-09-08','2026-09-09'],2],[['2026-09-08','2026-09-09',today],3],[['2026-09-08'],0],[[today],1],[['2026-09-08',today],1]])assert.equal(streak(dates,today),expected);
});
test('A5: v2 permite ayer y recalcula la racha, v1 lo rechaza',()=>{
  ...
  for(const date of ['2026-09-08','2026-09-11'])assert.throws(()=>markHabit(habits,'a',date,today,2),/hoy o ayer/);
...
test('Los días de calendario cruzan mes, año y febrero bisiesto',()=>{
  assert.equal(previousDay('2026-01-01'),'2025-12-31');assert.equal(previousDay('2024-03-01'),'2024-02-29');
...
test('Un fallo de almacenamiento no devuelve éxito ni modifica los hábitos previos',()=>{
  const before=createHabit([],'Leer','a');const next=markHabit(before,'a',today,today);
  assert.throws(()=>saveHabits({setItem(){throw new Error('QuotaExceeded');}},'demo',next),/QuotaExceeded/);
  assert.deepEqual(before[0].dates,[]);
});
test('No sobrescribe ni acepta datos corruptos al leer',()=>{
  for(const raw of ['{','{}',JSON.stringify({version:1,habits:[{id:'a',name:'Leer',dates:['2026-02-30']}]})]){
    let wrote=false;assert.throws(()=>readHabits({getItem:()=>raw,setItem:()=>{wrote=true;}},'demo'));
